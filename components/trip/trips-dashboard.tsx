  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <span className="text-body-md text-text-secondary">Loading trips…</span>
      </div>
    );
  }
    ...
    <div className="space-y-12">
      ...
      <section>
        ...
        {trips.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-border-subtle rounded-lg">
            ...
            <Link
              href="/trips/new"
              className="inline-block bg-primary text-on-primary px-6 py-2 rounded text-body-md font-medium hover:opacity-90 transition-opacity"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-spacing-gap-lg">
            {trips.slice(0, 4).map((trip) => (
              <Link
                key={trip.id}
                href={`/trips/${trip.id}`}
                className="group cursor-pointer border border-border-subtle hover:border-primary transition-all duration-300 p-spacing-base rounded-lg"
              >
                ...
                <div className="px-2 pb-2">
                  <h4 className="text-headline-md text-primary group-hover:text-primary transition-colors truncate">
                    {trip.title}
                  </h4>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    {trip.checkpoints?.slice(0, 3).map((cp) => (
                      <span key={cp.id} className="px-2 py-0.5 bg-hover-fill text-label-md rounded">
      ...
      <section>
        <div className="flex items-center justify-between mb-spacing-gap-lg">
          ...
          <Link
            href="/trips/new"
            className="bg-primary text-on-primary px-3 py-1 rounded text-label-md hover:opacity-90 transition-opacity flex items-center gap-1"
          >
        ...
        <div className="grid grid-cols-3 gap-spacing-gap-md h-[320px]">
          <Link
            href="/trips/new"
            className="col-span-2 relative group cursor-pointer overflow-hidden rounded-lg border border-border-subtle bg-surface flex items-center justify-center"
          >
          ...
          <div className="col-span-1 grid grid-rows-2 gap-spacing-gap-md">
            <Link
              href="/trips/new"
              className="relative group cursor-pointer overflow-hidden rounded-lg border border-border-subtle bg-surface flex flex-col justify-center items-center p-4 text-center"
            >
            ...
            <Link
              href="/trips/new"
              className="relative group cursor-pointer overflow-hidden rounded-lg border border-border-subtle bg-primary flex flex-col justify-center items-center p-4 text-center text-on-primary"
            >
