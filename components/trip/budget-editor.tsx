...
export function BudgetEditor({ tripId }: { tripId: string }) {
  ...
  function handleAdd() {
    ...
    const item: BudgetItem = {
      id: crypto.randomUUID(), tripId, checkpointId: null, category: category as BudgetItem["category"],
      amount: parsed.toString(), currency: "USD", isAutoGenerated: false,
    };
    addBudgetItem(item);
    setAmount("");
    setShowForm(false);
  ...
  return (
    <section className="border border-border-subtle rounded-lg bg-surface p-spacing-gap-md">
      ...
      {budgetItems.length === 0 && !showForm && <p className="mt-4 text-body-md text-text-secondary">No budget items yet. Add your first expense.</p>}
      <ul className="mt-4 space-y-1">
        {budgetItems.map((item) => (
          <li key={item.id} className="flex items-center justify-between px-3 py-2 text-body-md">
